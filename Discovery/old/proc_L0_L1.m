...
if strcmp(INSTRUMENT,'hsas')
   din    = [DOUT_HSAS DATESTR "/" DATESTR "_Calibrated/" ];     # <<<<<<<<<<< CHECK THIS
...
# here we should use functions name in input parameters to read gps data 
	fn_gps = glob([DIR_GPS, DATESTR, GLOB_GPS]){1};
	gps = FNC_RD_GPS(  fn_gps   );

	
#----------------------------------
### Read TILT AND ROLL data ###
	fn_ths = glob([DIR_ATT, DATESTR, GLOB_ATT]){1};
...
### Read WIND data ###
# here we should use functions name in input parameters to read gps data 
	fn_wind = glob([DIR_WIND, DATESTR, GLOB_WIND]){1};
...
if strcmp(INSTRUMENT,'hsas')
 ...
 %   catch
   ...
   # add the apparent wind data
	L0.appwind_spd = interp1(wind.time, wind.wspd, L0.time, TIME_INT_METHOD); # [m/s]
	L0.appwind_dir = interp1(wind.time, wind.wdir, L0.time, TIME_INT_METHOD); # [degrees]
	
	if isvarname('surf')
		fld = fieldnames(surf);
		for ifld = 2:length(fld)
			L0.surf_met.(fld{ifld}) = interp1(surf.time, surf.(fld{ifld}), L0.time, TIME_INT_METHOD); 
		endfor
	else
		if isfield(wind, 'airt') & isfield(wind, 'humid')
